"""Unit tests for the trae plugin against fixture records (T023).

trae is the pre-summarized path (FR-002/FR-013): each session_memory_*.jsonl
...
def test_parse_assembles_text_from_learned_and_outcome(memory_dir, type_map):
    path = write_memory(memory_dir, "-c-Code-proj--p2-abc", "20260912",
                        [record()])
    mat = trae.parse(SourceRef(source="trae", ref=f"{path}#L0", day=DAY))
...
def test_parse_maps_type_from_first_matched_field(memory_dir, type_map):
    ...
    mat2 = trae.parse(SourceRef(
        source="trae", ref=f"{outcome_only}#L0", day=date(2026, 9, 13)))
