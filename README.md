...
- **变异测试**默认只打"改坏了会**静默**出问题"的链路：宪法 VII 点名的手写核心
  ...
  当前基线：1370 个变异体
  被杀死、212 个存活、24 个无测试覆盖，**变异分数 86.6%**。
  ...
  **上面那份"存活 28 个逐条看过"的分类已经作废**，留在这里只当方法参考。
  ...
  212 条存活已按"该不该被杀掉"分诊完（规则化归类 + 抽样手工植入验证）：
  ...
  | **真缺口** | **0** | 三轮重建 + 20 条补测之后，这一桶清空了 |
  | codec 名大小写 | 6 | `"utf-8"` → `"UTF-8"`，Python 编解码器名不敏感，任何测试都杀不死 |
  | 文件系统大小写 | 4 | 如 `"scope.json"` → `"SCOPE.JSON"`。本机 APFS 大小写不敏感，所以读哪个名字都能读到，变异体杀不掉。**注意：换平台跑就能杀掉是推测，不是事实** —— CI 只跑提交门禁、从不跑 `make mutation`，所以这 4 条至今在任何环境都没被杀过。了结方式不是指望某台机器，而是补一条与文件系统无关的断言：`tests/unit/test_path_literals.py` 钉住文件名字面量本身，三条变异逐条手工打回源码验过（全杀）。**表里的 212/4 仍是上次重建的快照，这个数字要等下一次全量重建才会变** |
