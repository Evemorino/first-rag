...
- **变异测试**默认只打"改坏了会**静默**出问题"的链路：宪法 VII 点名的手写核心
  ...
  **上面那份"存活 28 个逐条看过"的分类已经作废**，留在这里只当方法参考。
  ...
  下面这张分诊表是按**上一轮**（1370/212/24）的存活名单归类的。2026-09-24 又跑了两次
  全量重建（kimi 重写 + 蒸馏分批 + 新模块进范围），存活数仍是 212，但**构成已经变了**：
  新增的 44 个变异体全部被杀，所以下面每桶的数字是"继承自上一轮"，不是本轮重测的。
  本轮只重新核了两处（见表中带 ✦ 的行）：那 4 条大小写变异体、以及新模块。其余待重判。

  | 类别 | 条数 | 含义 |
  |---|---|---|
  | 文案/输出格式 | 84 | prompt 话术、usage、日志串、排版参数。杀掉只能把整段文案抄进断言，成本高于收益 |
  | 待人工判 | 81 | 多为"参数置 None 后行为其实不变"（`cleanup_raw(now=None)` 函数内部会重算同样的值、`indent=None` 只影响排版），但每条要单独确认 |
  ...
  | **真缺口** ✦ | **0** | 说"清空了"说早了：把 `distill_batches` 纳入范围后，它**立刻**跑出 3 条真缺口（预算边界的 off-by-one：`>`→`>=`、初始与重置的 `0`→`1`），因为装箱测试从没让一批正好装满。已补 `[120, 120]` 断言并本轮确认 15/15 全杀。教训不是"缺口清完了"，而是**新代码不进 `only_mutate` 就等于没被检验过** |
  | codec 名大小写 | 6 | `"utf-8"` → `"UTF-8"`，Python 编解码器名不敏感，任何测试都杀不死 |
  | 文件系统大小写 ✦ | 4 | 如 `"scope.json"` → `"SCOPE.JSON"`。**这 4 条在两次独立全量重建里都判"存活"，而且mutmut 机制上就杀不掉**：它不逐字替换源码，而是给每个变异体生成一份完整副本（trampoline 分发），原函数体里的 `"scope.json"` 一直在文件里，所以"读源码断言字面量"的测试对它永远为真。`tests/unit/test_path_literals.py` 的实际价值是另一件事：人或 AI 真把一个字母改大写时会被抓到（手工逐条打回源码验过三次）。指望换平台杀掉是空的 —— CI 从不跑 `make mutation` |
