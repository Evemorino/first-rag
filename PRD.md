...
> **这三份当前是 Proposed —— 决策段已由 AI 起草候选稿（依用户 2026-09-26 授权）、等用户改定**
> （LG-005：AI 只提供素材与选项、结论由用户写；候选稿不算结论），所以**不上表**；
...
| **v0.7.13** | 2026-09-26 | **把 T066/T067/T077 三条【用户】自留项的候选措辞落盘（无 FR/NFR/AC 变更、`src/` 未改一行；LG-005 不变 —— 候选稿不算用户的结论）**。起因：这三条按 LG-001（Dogfood 首印象）/ LG-003（★ 任务的预测留痕）/ LG-005（实施期新增技术决策）只有用户本人能完成，长期是收口路径上唯一的阻塞。本次在用户明确授权下，AI 把**候选措辞**写进 `notes/t066-dogfood-material.md`（首印象）、`notes/t027-review.md`（LG-003 复盘）、`docs/adr/0012`–`0014`（三份 ADR 的 `### 候选稿` 子节），每处都标「AI 起草、待你改定」。**三份 ADR 状态仍是 Proposed**（候选稿不算结论），T066/T067/T077 仍未勾选；§7 表下指针与 `docs/adr/README.md` 的措辞同步。要收口仍需用户把自己的话写进去。 |
