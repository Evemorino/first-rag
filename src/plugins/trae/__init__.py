"""trae plugin: pre-summarized 轻转换路径（FR-002 / FR-013）。
    ...
    for path in sorted(MEMORY_DIR.glob(f"projects/*/{pattern}/session_memory_*.jsonl")):
        ...
        for index, line in enumerate(lines):
            ...
            try:
                ...
                continue
            refs.append(SourceRef(source="trae", ref=f"{path}#L{index}", day=day))
    return refs
...
PLUGIN = Plugin(name="trae", discover=discover, parse=parse)
