...
## 这三份现在是 Proposed（候选稿已起草，待用户改定）

**`## 决策（待你写）` 那一节仍等用户本人写**（宪法 §12.1 LG-005：实施中新增的
技术决策，AI 只提供素材与选项，结论由作者写）。每份文件里已经填好：背景、备选与被否的
理由、当时的证据、可逆性与已知代价，以及**三个具体的待答问题** —— 把答案写成那一段，
状态改成 Accepted 即可。

2026-09-26，用户授权 AI 先把**候选措辞**写进各文件的 `### 候选稿` 子节（每条都标了
「AI 起草、待你改定」）。**候选稿不算结论** —— 状态仍是 Proposed，你把它改成你自己的话
（删掉候选稿标记）之后才转 Accepted。
